...
<div class="row">
    <div class="col-xs-4">
        ...
        <input type="text" name="email" id="email" class="form-control" @if($user && $user != Auth::user()) value='{{$user->email}}' @elseif($ticket) value='{{$ticket->name}}' @endif />
        <label for="content">Текст сообщения:</label>
        <textarea name="content" id="content" class="form-control"></textarea>
        {{Form::close()}}
    </div>
    @if($user == Auth::user()) @elseif($ticket)
